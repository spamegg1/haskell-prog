...
meanMaybe vals =
    if any isNothing vals then Nothing
...
movingAvg [] _ = []
movingAvg vals n =
    if n /= length nextVals then []
    else meanMaybe nextVals : (movingAvg restVals) n
    where
...
movingAvgTS (TS [] []) _ = TS [] []
movingAvgTS (TS times values) n = TS times smoothedVals where
    nothings = replicate (n `div` 2) Nothing
    smoothedVals = mconcat [nothings, movingAvg values n, nothings]
